...
function loadPodcasts() {
    ...
    $.ajax({
        type: "GET",
        url: 'http://localhost:5000/f_episodes',
        success: function (data) {
            console.log(JSON.stringify(data));
            let dataDict = data["data"]
            Object.keys(dataDict).forEach(function (key) {
                $("#epDrop").append(`<option value = '${dataDict[key]}'> ${key} </option>`)
            });
        }
    });
}
...
$(document).ready(function () {
    ...
    $("#submit_podcast").click(function () {
        $("#content").append('<div class="full-sound"> <audio name="sound" controls controlsList="nodownload"> <source src="https://www2.cs.uic.edu/~i101/SoundFiles/PinkPanther60.wav" type="audio/wav"> </audio> </div> <input type="button" name="start_time" value="開始時刻"> → <label name="select_start_time"></label> <br> <input type="button" name="end_time" value="終了時刻"> → <label name="select_end_time"></label>');
    ...
    var createForm, config, maxLength, $form, $play, $pause, $stop, $save,
        ...
        initialPeaks, getRegionLengthText, toMinutesAndSeconds, round, padLeft, createWaveSurfer;
    ...
    var _songcut_config = {
         ...
         * URI of song.
         */
        file: 'https://s3.amazonaws.com/ui-podcast-bucket/startupEpisode1.mp3',

        /**
         * Form-data passed from initiator -> to this script -> to target endpoint.
         */
        state: {
            id: '',
            ...
            sourcePath: '/uploads/cache/28022017_210358_58b5e5be1e071.mp3'
        },
         ...
         */
        target: {
            uri: 'http://www.online-ringtone-maker.com/ComingSoon/release/cut.php',
            method: 'post'
        },

        /**
         * WaveSurfer player options.
         */
        player: {
            container: '#waveform',
            cursorColor: '#c63ab2',
   ...
   */
    ...
    $('select').on('change', function () {
        ...
        config.file = $('option:selected').val();
        wavesurfer.destroy()
        createWaveSurfer();
    });
